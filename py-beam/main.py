...
from commons import basename_backwards_x4, extract_files_from_dir
...
def arabic_remove_diac(text: str) -> str:
    if text is None:
        return text
...
def extract_arabic_words_with_filename(line_with_filename):
    ...
    return [(basename_backwards_x4(file_name), word) for word in arabic_tokenize_text(line, arabic_remove_diac)]

def extract_words_with_filename(line_with_filename):
    (file_name, line) = line_with_filename
    return [(basename_backwards_x4(file_name), word) for word in line.split(" ")]
...
def main(argv=None, save_main_session=True):
    ...
    with beam.Pipeline(options=pipeline_options) as p:
        ...
        if os.path.isfile(known_args.input):
            ...
            files = extract_files_from_dir(os.path.abspath(known_args.input), known_args.ext)
        else:
        ...
        words = (p 
            ...
            | 'Read lines' >> ReadAllFromText(with_filename=True)
            | 'Split' >> (
...
if __name__ == "__main__":
    ...
    logging.log(logging.INFO, 'Script End executed in {} sec.'.format(duree_exec))